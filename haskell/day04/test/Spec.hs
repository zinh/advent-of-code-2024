...
import Test.HUnit
import Day04.Grid (Grid, Position, Direction, inBounds, charAt, checkXMAS)
...
charAtTests = TestList
  [ TestLabel "Valid position middle" $ TestCase $ do
      assertEqual "Should get 'M' at position (0,1)"
        (Just 'M') (charAt testGrid (0,1))

  , TestLabel "Valid position top-left" $ TestCase $ do
      assertEqual "Should get 'X' at position (0,0)"
        (Just 'X') (charAt testGrid (0,0))

  , TestLabel "Valid position bottom-right" $ TestCase $ do
      assertEqual "Should get 'S' at position (3,3)"
        (Just 'S') (charAt testGrid (3,3))
...
checkXMASTests :: Test
checkXMASTests = TestList
  [ TestLabel "Horizontal XMAS" $ TestCase $ do
      let direction = [(0,1), (0,2), (0,3)]
      assertEqual "Should find XMAS horizontally"
        True (checkXMAS testGrid (0,0) direction)

  , TestLabel "Vertical XMAS" $ TestCase $ do
      let direction = [(1,0), (2,0), (3,0)]
      assertEqual "Should find XMAS vertically"
        True (checkXMAS testGrid (0,0) direction)

  , TestLabel "Not XMAS (wrong start)" $ TestCase $ do
      let direction = [(0,1), (0,2), (0,3)]
      assertEqual "Should not find XMAS when starting with wrong letter"
        False (checkXMAS testGrid (0,1) direction)

  , TestLabel "Not XMAS (incomplete pattern)" $ TestCase $ do
      let direction = [(0,1), (0,2), (0,3)]
      assertEqual "Should not find XMAS when pattern is incomplete"
        False (checkXMAS testGrid (1,0) direction)

  , TestLabel "Out of bounds" $ TestCase $ do
      let direction = [(0,1), (0,2), (0,3)]
      assertEqual "Should not find XMAS when pattern goes out of bounds"
        False (checkXMAS testGrid (3,1) direction)

  , TestLabel "Empty grid" $ TestCase $ do
      let direction = [(0,1), (0,2), (0,3)]
      assertEqual "Should not find XMAS in empty grid"
        False (checkXMAS emptyGrid (0,0) direction)
  ]
